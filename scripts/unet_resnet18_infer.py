"""
unet_resnet18_infer.py
----------------------
Multi-scale UNet inference on visible-light sea ice images using a ResNet18 encoder
(segmentation-models-pytorch). Weights must be trained with unet_resnet18_colab.ipynb
and placed at:
...
Outputs go to:
  outputs/results/batch_multiscale/{folder}/{image_stem}/
    ...
    pa_scaling.png        — log-log perimeter-area plot + fractal dimension D
    floe_labels.npy       — 2D int32 label array (0=sea, 1..N=floe ID)
    floe_stats.json       — shape metrics + fractal dimension metadata
"""
...
# CLAHE preprocessing — must match training settings in unet_resnet18_colab.ipynb
USE_CLAHE  = True
CLAHE_CLIP = 2.0
CLAHE_GRID = (8, 8)
...
def main():
    if not WEIGHTS_PATH.exists():
        ...
        print("Train the model first:  see scripts/training/unet_resnet18_colab.ipynb")
        return
