"""
unet.py — Visible RGB UNet training (single image)
====================================================
Trains a small UNet model on a single annotated visible-light sea ice image.
The image is tiled into a grid (e.g. 500x500 tiles), split into train/val/test
sets, and used to train a 3-level encoder-decoder UNet from scratch.

For training on multiple images, use unet_multiscale.py instead.
...
-------------------------------------
    IMAGE_PATH    Path to the source RGB image (4000x3000 recommended)
    MASK_PATH     Path to the corresponding CVAT-exported mask PNG
    OUT_DIR       Output directory for tiles and weights
    TILE_SIZE     Tile crop size in pixels (default 500)
    STRIDE        Tile overlap stride (default 250)
    EPOCHS        Number of training epochs (default 75)
    LR            Learning rate (default 1e-3)

Inputs
------
    A single image + mask pair from data/images/

Outputs
-------
    outputs/dataset_tiles/
