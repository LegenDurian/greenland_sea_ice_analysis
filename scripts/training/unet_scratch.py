...
unet_scratch.py
---------------
UNet training from scratch for visible-light sea ice images.

Tiles a single image + mask pair into 500×500 crops, splits them into
train / val / test sets, and trains a UNetSmall model using BCE + Dice loss.

Key differences from unet_multiscale.py
────────────────────────────────────────
  Tiles are pre-generated and saved to disk before training (not on-the-fly).
  Works with a single image + mask pair (configured via IMAGE_PATH / MASK_PATH).
  Use unet_multiscale.py when you have multiple annotated images.

Config (top of file):
  IMAGE_PATH — path to your input RGB image (JPEG or PNG)
  MASK_PATH  — path to the corresponding binary mask (same resolution)
  OUT_DIR    — where tiles and best_unet.pt are saved

Outputs:
  outputs/dataset_tiles/best_unet.pt   — best weights (val Dice)
"""
...
# Config (edit these)
# ----------------------------
_DATA_DIR  = Path(__file__).parent.parent.parent / "data" / "images"
IMAGE_PATH = str(_DATA_DIR / "unet_test_ice_2.JPG")       # your 4000x3000 RGB image (or JPG/PNG)
MASK_PATH  = str(_DATA_DIR / "unet_test_ice_mask_2.png")  # CVAT exported mask (same size as image)
OUT_DIR    = str(Path(__file__).parent.parent.parent / "outputs" / "dataset_tiles")  # output folder
