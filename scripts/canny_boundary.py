...
canny_boundary.py
-----------------
Canny edge detection with boundary filtering for sea ice images.

What it does:
  1) Bilateral smoothing -> Canny edges
  2) Otsu threshold to get an ice mask (ice ~ bright, water ~ dark)
  3) Morphological gradient of the mask = thin boundary band
  4) Keep only edges that lie on the mask boundary (removes inside-ice texture edges)
